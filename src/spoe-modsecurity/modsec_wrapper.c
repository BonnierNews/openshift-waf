...
{
	LOG(&null_worker, "MODSECURITY: %s", str);
}
...
int modsecurity_process(struct worker *worker, struct modsecurity_parameters *params)
{
	...
	LOG(worker, "Modsecurity header check returned: %d",(int) status);
	if (status != DECLINED && status != DONE)
		return_code = status;

	/* Process request body analysis.*/
	status = modsecProcessRequestBody(req);
	LOG(worker, "Modsecurity body check returned: %d", (int) status);
	if (status != DECLINED && status != DONE && status != AP_NOBODY_READ)
		return_code = status;
